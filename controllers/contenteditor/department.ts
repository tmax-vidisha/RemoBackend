...
const Department_Id = "4d933ed8-bce3-4429-9af6-8e509eb6d2dc";
    ...
    if (!token) {
      ...
      try {
        const response = await fetch(
          `${BASE_PATH}/${Site_Id}/lists/${Department_Id}/items`,
          {
            method: "POST",
            headers: {
...
const getLatestDepartment = asyncHandler(
  async (req: Request, res: Response) => {
    console.log(
    ...
    if (!token) {
      ...
      const response =
        // await axios.get('https://graph.microsoft.com/v1.0/me/events?$select=subject,body,bodyPreview,organizer,attendees,start,end,location', {
        await axios.get(
          `${BASE_PATH}/${Site_Id}/lists/${Department_Id}/items?$expand=fields&$orderby=lastModifiedDateTime desc`,
          {
            headers: {
              Authorization: `Bearer ${token} `,
              "Content-Type": "application/json",
...
export { postTableDepartment, getLatestDepartment };
